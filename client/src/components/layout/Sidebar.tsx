...
export function Sidebar({ onSelect, onNew, selectedId }: SidebarProps) {
  ...
  const [searchResults, setSearchResults] = useState<Inspiration[] | null>(null);

  // Server-side semantic search when query changes
  useEffect(() => {
    if (!query.trim()) {
    ...
    const timer = setTimeout(async () => {
      try {
        const result = await api.searchInspirations(query);
  ...
  const displayList = searchResults ?? inspirations;

  const label = (item: Inspiration) => item.summary || item.content.slice(0, 30) || '无标题';
      ...
      <nav className="flex-1 overflow-y-auto px-4 pb-4">
        {displayList.length === 0 ? (
          <p className="text-[12px] text-[var(--text-quaternary)] mt-2">
            {query ? '没有找到相关灵感' : inspirations.length === 0 ? '还没有灵感记录' : ''}
          </p>
        ) : (
